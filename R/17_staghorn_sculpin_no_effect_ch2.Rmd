---
title: "staghorn sculpin mixing models with MixSIAR"
...
#load_mix_data
mix <- load_mix_data(filename=here("data", "03_clean data", "staghorn_sculpin.csv"),
iso_names=c("d15N","d13C"), factors=NULL,fac_random=NULL,
fac_nested=NULL, cont_effects=NULL)

#load_source_data
source <- load_source_data(filename=here("data", "03_clean data", "sources.csv"),
source_factors=NULL, conc_dep=FALSE, data_type = "raw", mix)

#Load discrimination data
discr <- load_discr_data(filename=here("data", "03_clean data", "TEF.csv"), mix)
...
jags.1 <- run_model(run="very long",mix,source,discr,model_filename,
 ...
 # save model fit so that it can be called up again without having to be run again
  saveRDS(jags.1, here("model_output","staghorn_sculpin", "no effects", "staghorn_sculpin.model.no.effect.rds"))
...
output jags source code to change posterior plot
```{r output jags source code to change posterior plot}
# read the model fit back in later to fiddle with plotting
jags.1<-readRDS(here("model_output", "staghorn_sculpin", "no effects", "staghorn_sculpin.model.no.effect.rds"))
...
if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
  ...
  if(mix$n.fe == 0){ # only if there are no fixed effects, otherwise p.global is meaningless
    ...
    my.title <- "Staghorn Sculpin"
    print(ggplot2::ggplot(df, ggplot2::aes(x=x, colour=sources, fill=sources)) +
            ggplot2::geom_density(alpha=.3, ggplot2::aes(y=..scaled..)) +
            ggplot2::theme_classic() +
            #ggplot2::scale_colour_manual(values = cols,aesthetics = "colour") +
            ...
            ggplot2::scale_x_continuous(expand = c(0, 0),
                                        ...
                                        labels = c("0.00","0.25","0.5","0.75","1.00   ")) +            ggplot2::scale_y_continuous(expand = c(0, 0),limits = c(0,1.05)) +
            ggplot2::labs(title = my.title) +
            ggplot2::theme(legend.position=c(0.5,1), legend.justification=c(1,1),
...
#        geom_density(alpha=.3) +
  ...
  ggplot2::ylab("Scaled Posterior Density") +
  #ggplot2::labs(title = my.title) +
  ggplot2::theme(legend.position="right"), legend.justification=c(1,1), legend.title=ggplot2::element_blank())
...
#        geom_density(alpha=.3) +
  ...
  ggplot2::theme(legend.position="right"), legend.justification=c(1,1), legend.title=ggplot2::element_blank())   # + xlim(0,2)
