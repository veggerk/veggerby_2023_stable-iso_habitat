---
title: "shiner perch mixing models with MixSIAR"
...
#load_mix_data
mix <- load_mix_data(filename=here("data", "03_clean data", "shiner_perch.csv"),
...
jags.1 <- run_model(run="very long",mix,source,discr,model_filename,
 ...
 # save model fit so that it can be called up again without having to be run again
  saveRDS(jags.1, here("model_output","shiner_perch", "no effect", "shiner_perch.model.no.effect.rds"))
...
jags.1<-readRDS(here("model_output", "shiner_perch", "no effect", "shiner_perch.model.no.effect.rds"))
...
if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
  ...
  if(mix$n.fe == 0){ # only if there are no fixed effects, otherwise p.global is meaningless
    ...
    print(ggplot2::ggplot(df, ggplot2::aes(x=x, colour=sources, fill=sources)) +
            ...
            ggplot2::xlab("") +
            ggplot2::ylab("") +
            ggplot2::scale_x_continuous(expand = c(0, 0),limits = c(0,1.05)) +
            ggplot2::scale_y_continuous(expand = c(0, 0),limits = c(0,1.05)) +
            ggplot2::labs(title = my.title) +
            ggplot2::theme(legend.position=c(0.5,1), legend.justification=c(0.5,1),
                           legend.title=ggplot2::element_blank()) +
            ggplot2::theme(legend.text=element_text(size=15)))
